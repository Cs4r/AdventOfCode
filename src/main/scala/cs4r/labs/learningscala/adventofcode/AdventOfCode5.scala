...
object AdventOfCode5 extends App {
  ...
  val puzzleInput = """zgsnvdmlfuplrubt
                      |vlhagaovgqjmgvwq
                      |ffumlmqwfcsyqpss
                      |zztdcqzqddaazdjp
                      |eavfzjajkjesnlsb
                      ...
                      |pxdfvcpvwaddrzwv
                      |phdqqxleqdjfgfbg
                      |cqfikbgxvjmnfncy""".stripMargin

  private val listOfStrings: List[String] = puzzleInput.split("\n").toList

  def containsStrings(mandatory: String)(input: String): Boolean = {
  ...
  def secondCondition(input: String) = {
    input.map(e => e.toString + e.toString).exists(e => input.contains(e))
  }


  def thirdCondition(input: String) = {
    List("ab", "cd", "pq", "xy").forall(e => !input.contains(e))
  }

  val input = "dvszwmarrgswjxmb"
  println(firstCondition(input))
  println(secondCondition(input))
  println(thirdCondition(input))
