...
object AdventOfCode8 extends App {
  ...
  val puzzleInput =
    """"sjdivfriyaaqa\xd2v\"k\"mpcu\"yyu\"en"
      ...
      |"ccxjgiocmuhf\"ycnh"
      |"lltj\"kbbxi"""".stripMargin

  val listOfStrings = puzzleInput.split("\n").toList
  ...
  val listOfTestStrings = testInput.split("\n").toList

  def partA(l: List[String]): Int = {
    l.map(x => charactersOfCode(x) - charactersInMemory(x)).sum
  }

  def charactersOfCode(s: String) = s.length

  def charactersInMemory(s: String) = {
    val withoutQuotes = s.drop(1).dropRight(1)
    val withoutSlashes = withoutQuotes.replaceAll("\\\\\"|\\\\\\\\", "*")
    withoutSlashes.replaceAll("\\\\x\\p{XDigit}{2}", "*").size
  }

  def partB(l: List[String]): Int = {
    l.map(x => charactersOfEncodedRepresentation(x) - charactersOfCode(x)).sum
  }

  def charactersOfEncodedRepresentation(s: String) = {
    val encoded = s.map(_ match {
      case '"' => "**"
      case '\\' => "**"
      case '\"' => "**"
      case default => default
    }).reduce(_.toString + _.toString).toString

    charactersOfCode(encoded) + 2
  }


  println("Part A")
  println(partA(listOfStrings))
//  for (i <- listOfTestStrings) {
//    println(s"testString $i, charactersOfCode = ${charactersOfCode(i)}, charactersInMemory = ${charactersInMemory(i)}")
//  }


  println("Part B")
  println(partB(listOfStrings))
//  for (i <- listOfTestStrings) {
//    println(s"testString $i, charactersOfCode = ${charactersOfCode(i)}, charactersOfEncodedRepresentation = ${charactersOfEncodedRepresentation(i)}")
//  }
}
