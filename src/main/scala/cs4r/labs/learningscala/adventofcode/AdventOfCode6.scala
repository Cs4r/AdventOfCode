...
object AdventOfCode6 extends App {
  ...
  val listOfInstructions: List[String] = puzzleInput.split("\n").toList

  val regexPattern = "([A-Za-z ]+)([0-9]+,[0-9]+)[A-Za-z ]+([0-9]+,[0-9]+)".r

  type Grid = Array[Array[Int]]

  val newGrid : () => Grid = () => Array.ofDim[Int](1000, 1000)

  // Curried so that I can reuse it later (see below)
  def traverseGrid(from: (Int, Int), to: (Int, Int), grid: Grid)(f: Int => Int) = {

    for (i <- from._1 to to._1)
      for (j <- from._2 to to._2)
        grid(i)(j) = f(grid(i)(j))
  ...
  type LightAction = ((Int, Int), (Int, Int), Grid) => Grid

  val turnOn1: LightAction = (from, to, grid) => traverseGrid(from, to, grid)(_ => 1)

  val turnOff1: LightAction = (from, to, grid) => traverseGrid(from, to, grid)(_ => 0)

  val toggle1: LightAction = (from, to, grid) => traverseGrid(from, to, grid)(e => if(e == 1) 0 else 1 )

  val stringToTuple : String => (Int, Int) = input  => {
  ...
  // Curried so that I can reuse it later (see below)
  def followSantaInstructions(instructions: List[String], grid: Grid)(turnOn: LightAction)(turnOff:LightAction)(toogle:LightAction): Grid = instructions match {
    case h::tail => {
      val regexPattern(action, first, second) = h
      val from = stringToTuple(first)
      val to = stringToTuple(second)

      val updatedGrid = action.trim match {
        case "turn on" => turnOn(from, to, grid)
        case "turn off" =>  turnOff(from, to, grid)
        case "toggle" => toogle(from, to, grid)
      }


      followSantaInstructions(tail, updatedGrid)(turnOn)(turnOff)(toogle)
    }
    case Nil => grid
  }

  val countLitLigths : Grid => Int  = grid => {
    (grid.toList map (row => row.toList.count(_==1))).sum
  }


  def followSantaInstructions1(instructions: List[String], grid: Grid) : Grid = followSantaInstructions(instructions, grid)(turnOn1)(turnOff1)(toggle1)

  println(countLitLigths(followSantaInstructions1(listOfInstructions, newGrid())))


  // Second Part
  val turnOn2 : LightAction = (from, to, grid) =>  traverseGrid(from, to, grid)(_ + 1)

  val turnOff2 : LightAction = (from, to, grid) => traverseGrid(from, to, grid)(e =>  if(e-1 < 0)  0 else e-1 )

  val toggle2 : LightAction = (from, to, grid) =>  traverseGrid(from, to, grid)(_ + 2 )

  def followSantaInstructions2(instructions: List[String], grid: Grid) : Grid = followSantaInstructions(instructions, grid)(turnOn2)(turnOff2)(toggle2)

  val countBrightness : Grid => Int = grid => {
    (grid.toList map (row => row.toList.sum)).sum
  }

  println(countBrightness(followSantaInstructions2(listOfInstructions, newGrid())))
